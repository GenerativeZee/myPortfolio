...
2. REIMS — Enterprise Plant SOP Agent: RAG agent on Azure Databricks with Databricks Vector Search, giving citable answers over structured/unstructured SOPs across multiple plant sites. Designed a full agent memory system — procedural (how to plan and act), semantic (SOP facts and domain knowledge), and episodic (past interactions) — persisted in Lakebase, plus site-isolated retrieval. Closed the feedback loop so the agent keeps improving post-deployment: thumbs up/down → MLflow traces → SME-reviewed eval set → DSPy prompt optimization → LLM-as-Judge → MLflow versioning, no redeploy. p95 latency under 4s, groundedness ≥0.85, citation completeness ≥0.95, zero cross-site data leaks.
