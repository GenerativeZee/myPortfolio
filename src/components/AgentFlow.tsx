  ...
  return (
    <div className="flex flex-col md:flex-row md:items-center w-full">
      {steps.map((step, i) => (
        <div
          key={i}
          className="flex flex-col md:flex-row md:items-center md:flex-1 last:md:flex-none"
        >
          {/* Node */}
            ...
            {step.parallel ? (
              ...
              >
                {step.parallel.map((p, j) => (
                  <span
                    key={j}
                    className="px-3 py-1.5 rounded-lg text-[11px] font-medium text-zinc-300 bg-white/[0.03] border border-white/[0.06] whitespace-nowrap text-center"
                  >
                    {p}
                  </span>
                ))}
              ...
              <div
                className={`px-4 py-2.5 rounded-xl border ${colors.border} ${colors.bg} ${colors.text} text-xs font-semibold whitespace-nowrap`}
              >
                {step.label}
              </div>
            )}
          ...
          {/* Connector */}
          {i < steps.length - 1 && (
            <div className="flex items-center justify-center shrink-0 my-1.5 md:my-0 md:mx-1 md:flex-1 md:min-w-[24px]">
